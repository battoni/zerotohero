<template>
  <div class="flex flex-col gap-5 pt-4">
      ...
      <aside class="flex flex-col gap-4">
        <div v-if="lists?.incoming.length" class="flex flex-col gap-3 rounded-card bg-sun-soft p-4" data-testid="friend-requests">
          ...
          <div v-for="p in lists.incoming" :key="p.id" class="flex flex-wrap items-center gap-2">
            ...
            <UiBtn size="sm" variant="mint" :disabled="busy" :data-testid="`accept-${p.handle}`" @click="respond(p.id, true)">{{ $t('friends.accept') }}</UiBtn>
            <UiBtn size="sm" variant="ghost" :disabled="busy" @click="respond(p.id, false)">{{ $t('friends.decline') }}</UiBtn>
          </div>
        </div>
        ...
        <div class="flex flex-col gap-3 rounded-card bg-surface p-4 shadow-soft">
          ...
          <div v-for="p in results" :key="p.id" class="flex items-center gap-2">
            ...
            <UiBtn v-else size="sm" variant="primary" :disabled="busy" :data-testid="`add-${p.handle}`" @click="add(p.id)">{{ stateOf(p.id) === 'incoming' ? $t('friends.accept') : $t('friends.add') }}</UiBtn>
          </div>
        </div>
    ...
    <UiConfirm
      :open="!!pendingRemove"
      :title="pendingRemove ? $t('friends.removeTitle', { name: pendingRemove.displayName || pendingRemove.handle }) : ''"
      :confirm-label="$t('friends.removeYes')"
      :busy="busy"
...
const pendingRemove = ref<Profile | null>(null)
...
async function confirmRemove() {
  ...
  if (!p) return
  await act(() => repo.removeFriend(p.id))
